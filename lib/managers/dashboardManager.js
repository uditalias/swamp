...
module.exports = EventDispatcher.define({
    ...
    initialize: function() {
        ...
        this._bindPresetsManagerEvents();
    ...
    _onSocketConnection: function(socket) {
        ...
        socketClient.on('preset.run', this._onClientRunPresetRequest, this);
        socketClient.on('preset.create', this._onClientCreatePresetRequest, this);
        socketClient.on('preset.delete', this._onClientDeletePresetRequest, this);
    ...
    _onClientRunPresetRequest: function(presetId) {
        this.presetsManager.runPreset(presetId);
    },

    _onClientCreatePresetRequest: function(preset) {
        this.presetsManager.createPersistentPreset(preset);
    },

    _onClientDeletePresetRequest: function(presetId) {
        this.presetsManager.deletePersistentPreset(presetId);
    },
    ...
    _onSocketDisconnected: function (socketClient) {
        ...
        socketClient.un('preset.run', this._onClientRunPresetRequest, this);
        socketClient.un('preset.create', this._onClientCreatePresetRequest, this);
        socketClient.un('preset.delete', this._onClientDeletePresetRequest, this);
    ...
    broadcast: function(message, channel) {
        if(channel) {
            this.io.sockets.in(channel).emit('message', message);
        } else {
    ...
    _sendSwampInitialData: function(clientId) {
        ...
        if(client) {
            ...
            var presets = this.presetsManager.serialize();
            var swamp = this.swampManager.serialize();

            client.sendMessage({ event: 'swamp.initialData', data: { services: services, commands: commands, presets: presets, swamp: swamp } });
        }
    },
    ...
    _bindPresetsManagerEvents: function() {

        this.presetsManager.on('created', this._onPresetCreated, this);

        this.presetsManager.on('deleted', this._onPresetDeleted, this);
    ...
    _onPresetCreated: function(preset) {
        this.broadcast({ event: 'preset.created', data: preset.serialize() });
    },

    _onPresetDeleted: function(presetId) {
        this.broadcast({ event: 'preset.deleted', data: presetId });
    },
    ...
    _onSwampServiceError: function(serviceName, data) {
        ...
        var channel = 'service_log_error_';
        ...
        this.broadcast({ event: 'service.error', data: { name: serviceName, log: data } }, channel);
    },

    _onSwampServiceOut: function(serviceName, data) {

        var channel = 'service_log_out_';
        var service = this.swampServicesManager.getByName(serviceName);
        ...
        this.broadcast({ event: 'service.out', data: { name: serviceName, log: data } }, channel);
    },
