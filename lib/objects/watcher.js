...
module.exports = Class.define({
    ...
    start: function() {
        if(!this._FSwatcher) {
            gaze(this._watchDir, { interval: 5007 }, this._onGazeWatcherStart.bind(this));
        }
    },
    ...
    _onGazeWatcherStart: function(err, watcher) {
        if(err) {
            this.callback && this.callback(err);
            return;
        }
        this._FSwatcher = watcher;
        this._FSwatcher.setMaxListeners(0);
    ...
    _fireChangeEvent: function(event, file) {
        this._timer = null;
        this.callback && this.callback(null, this._changes);
        this._changes = [];
    }
});