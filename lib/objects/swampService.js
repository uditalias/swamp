...
module.exports = EventDispatcher.define({
    $config: {
        ...
        inject: ['env', 'utils', 'environmentsManager', 'optionsManager', 'mainLoggersManager', 'logOptionsParser'],
    ...
    start: function(envName) {
        ...
        if(!this.isRunning) {
            ...
            this.utils.log('[{0}] `{1}` is running'.format(this.pid, this.name), this.utils.LOG_TYPE.SUCCESS, false, false, [this.mainLoggersManager.getOut(), this._loggers.out]);
    ...
    stop: function() {
        ...
        if(this.isRunning) {
            ...
            this.utils.log('[{0}] `{1}` has stopped. (started {2} ago)'.format(this.pid, this.name, this.utils.timeFrom(this.startTime)),
    ...
    restart: function(envName) {
        this.utils.log('Restarting `{0}`...'.format(this.name), this.utils.LOG_TYPE.INFO, false, false, [this.mainLoggersManager.getOut(), this._loggers.out]);
    ...
    _onWatcherEvent: function(err, changelog) {
        ...
        if(err) {
            ...
            this.utils.log('restartOnChange will not work for this service session, stopping the watcher.', this.utils.LOG_TYPE.ERROR, false, false, this._loggers.err);
            this._pathWatcher.stop();
            return;
        }
