...
module.exports = EventDispatcher.define({
    ...
    stop: function() {
        if(this.isRunning) {
            ...
            this.utils.log('[{0}] `{1}` has stopped. (started {2} ago)'.format(this.pid, this.name, this.utils.timeFrom(this.startTime)), this.utils.LOG_TYPE.INFO, false, false, this.mainLoggers.out);
    ...
    _initializeLoggers: function() {
        ...
        this.utils.mkfile(folderName + '/out.log', 'a');
        this.utils.mkfile(folderName + '/err.log', 'a');

        this._loggers = {
            out: this._createLogger({ filename: path.resolve(process.cwd(), folderName + '/out.log'), json: false }, true, this._options.maxLogsToSave),
            err: this._createLogger({ filename: path.resolve(process.cwd(), folderName + '/err.log'), json: false }, true, this._options.maxLogsToSave)
        }
    },
