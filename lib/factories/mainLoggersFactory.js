...
module.exports = Class.define({
    ...
    _initializeLoggers: function() {
        if(!this._loggers) {
            ...
            this._loggers = {
                out: this._createLogger({ filename: path.resolve(process.cwd(), 'logs/out.log'), json: false }),
                err: this._createLogger({ filename: path.resolve(process.cwd(), 'logs/err.log'), json: false })
            }
        }
    }
